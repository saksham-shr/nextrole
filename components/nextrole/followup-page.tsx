    ...
    <div className="space-y-6">
      <div className="mb-8">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--muted-foreground)] sm:text-base">
          Draft professional follow-up messages tailored to your role and stage. Copy and send — never auto-sent.
        </p>
      </div>
      ...
      {jobs.length > 0 && (
        <>
          ...
          <Surface className="p-5">
            <SectionTitle title="Select job" subtitle="Jobs you have applied to or are interviewing for" />
              ...
              {jobs.map((j) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-bold">{j.title} — {j.company}</p>
                      {j.archetype && (
          ...
          {selectedJob && (
            <Surface className="p-5">
              <SectionTitle
                title="Message type"
                subtitle={hasProvider ? "Pick a type to generate instantly, or copy the prompt for any AI" : "No API key — use Copy prompt to draft manually"}
              />
          ...
          {draft && (
            <Surface tone="ok" className="p-5">
              <div className="mb-3 flex items-center justify-between gap-3">
                <SectionTitle title={FOLLOWUP_TYPE_LABELS[activeType!]} subtitle="Ready to copy and send" />
