  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", gap: 16, maxWidth: 320, margin: "0 auto" }}>
          {[
            { label: "Evaluate any role", desc: "Fit score and decision in seconds." },
            { label: "Tailor your resume", desc: "AI-aligned to the exact job description." },
            { label: "Prepare autofill answers", desc: "Draft every answer before you open the form." },
            { label: "Track your pipeline", desc: "Saved → Evaluated → Applied → Offer." },
          ].map((it) => (
            <div key={it.label} className="nr-card" style={{ padding: "14px 18px", display: "flex", gap: 12, alignItems: "flex-start" }}>
              ...
              <div>
                ...
                <div className="nr-helper" style={{ marginTop: 2 }}>{it.desc}</div>
              </div>
            </div>
          ))}
        </div>
        <div style={{ fontSize: 12, color: "var(--muted-foreground-2)" }}>© 2026 Braevity</div>
      </div>
...
export function LoginPage({ error, message }: { error?: string; message?: string }) {
  ...
  return (
    <AuthLayout>
      ...
      <p style={{ fontSize: 14, color: "var(--muted-foreground)", marginBottom: 24 }}>Sign in to your Braevity account</p>
...
export function SignupPage({ error }: { error?: string }) {
  ...
  return (
    <AuthLayout>
      ...
      <p style={{ fontSize: 14, color: "var(--muted-foreground)", marginBottom: 24 }}>Apply faster with Braevity.</p>
