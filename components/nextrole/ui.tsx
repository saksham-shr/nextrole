...
export function Eyebrow({
  ...
  return (
    <p
      className={cx(
        "text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--muted-foreground)]",
        className,
      )}
    >
      {children}
  ...
  return (
    <div className="mb-4 flex items-start justify-between gap-4">
      <div>
        ...
        {subtitle ? (
          <p className="mt-1 text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--muted-foreground)]">
            {subtitle}
          </p>
        ) : null}
      </div>
...
export function Badge({
  ...
  return (
    <span
      className={cx(
        "inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.05em]",
        styles[tone],
        className,
      )}
    >
...
export function StatCard({
  ...
  return (
    <Surface tone={tone} className="p-4">
      ...
      {sublabel ? (
        <p className="mt-2 text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--muted-foreground)]">
          {sublabel}
        </p>
      ) : null}
    </Surface>
          ...
          <thead className="bg-[var(--surface-soft)]">
            <tr>
              {columns.map((column) => (
                <th
                  key={column}
                  className="border-b border-[var(--line)] px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--muted-foreground)]"
                >
                  {column}
                </th>
              ))}
    ...
    <label className="block">
      ...
      {textarea ? (
        <textarea
          ...
          className="w-full rounded-[8px] border border-[var(--line)] bg-[var(--surface)] px-3 py-2.5 text-[13px] outline-none placeholder:text-[var(--muted-foreground-2)] focus:border-[var(--accent)]"
        />
      ) : (
        <input
          name={name}
      ...
      {hint && (
        <p className="mt-1 text-[11px] font-semibold uppercase tracking-[0.05em] text-[var(--muted-foreground)]">{hint}</p>
      )}
    </label>
  );
}
    ...
    <div className="flex flex-wrap gap-2">
      {items.map((item) => (
        <button
          ...
          className={cx(
            "rounded-full border px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.05em]",
            active === item
              ? "border-[var(--accent)] bg-[var(--accent)] text-[var(--surface)]"
